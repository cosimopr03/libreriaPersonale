...
public class LibreriaUISwing extends AbstractUi
{
    ...
    public void avvia()
    {
        ...
        row1.add(btnRemove); row1.add(btnSortTitolo); row1.add(btnSearch);
        JPanel row2 = new JPanel(new GridLayout(1,3,5,5));
        row2.add(btnAdd); row2.add(btnModify); row2.add(btnSortAutore);

        JPanel actionPanel = new JPanel(new GridLayout(2,1,5,5));
        actionPanel.add(row1); actionPanel.add(row2);
        ...
        btnResetFiltri.addActionListener(e -> { cbFiltroStato.setSelectedIndex(0); cbFiltroValutazione.setSelectedIndex(0); applyFilters(); });
    ...
    {
        try { archivio.caricaLibri(); allBooks = archivio.getLibri(); popolaTabella(allBooks); }
    ...
    private void applyFilters()
    {
        ...
        List<Libro> filtered = allBooks.stream()
                ...
                .filter(l -> {
                    ...
                    if ("Non valutato".equals(fv)) return l.getValutazione() == null || l.getValutazione()==Valutazione.nonValutato;
    ...
    private void addBook()
    {
        ...
        if(JOptionPane.showConfirmDialog(frame,prompts,"Aggiungi Libro",JOptionPane.OK_CANCEL_OPTION)==JOptionPane.OK_OPTION){
            if(tfTit.getText().isBlank()||tfAut.getText().isBlank()||tfEd.getText().isBlank()||tfIsbn.getText().isBlank()){
                JOptionPane.showMessageDialog(frame,"I campi Titolo, Autori, Editore e ISBN sono obbligatori.","Errore",JOptionPane.ERROR_MESSAGE);return;}
            try{
    ...
    {
        int sel=table.getSelectedRow(); if(sel<0) return; String isbn=(String)tableModel.getValueAt(sel,0);
        try{ archivio.rimuoviLibro(isbn); loadAll(); }catch(IOException e){ showError(e);} }

    private void modifySelected(){
        int sel=table.getSelectedRow(); if(sel<0) return; String isbn=(String)tableModel.getValueAt(sel,0);
        String[] opts={"Stato","Valutazione"}; int ch=JOptionPane.showOptionDialog(frame,"Cosa modificare?","Modifica",JOptionPane.DEFAULT_OPTION,JOptionPane.QUESTION_MESSAGE,null,opts,opts[0]);
        try{
            if(ch==0){ String ns=(String)JOptionPane.showInputDialog(frame,"Nuovo stato:","Modifica Stato",JOptionPane.PLAIN_MESSAGE,null,new String[]{"NON_LETTO","IN_LETTURA","LETTO"},"LETTO"); if(ns!=null) archivio.modificaStato(isbn,ns);}
            else if(ch==1){ String nv=(String)JOptionPane.showInputDialog(frame,"Nuova valutazione:","Modifica Valutazione",JOptionPane.PLAIN_MESSAGE,null,new String[]{"1","2","3","4","5","Non valutato"},"3"); if(nv!=null&&!"Non valutato".equals(nv)) archivio.modificaValutazione(isbn,nv);}
            loadAll();
        }catch(IOException e){ showError(e);} }
    ...
    { String q=JOptionPane.showInputDialog(frame,"Ricerca titolo :");
        if(q==null) return;
        try{ popolaTabella(archivio.cerca(q)); }
        catch(IOException e){ showError(e);}
    }
